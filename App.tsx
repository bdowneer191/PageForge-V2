...
import { Recommendation, Session, CleaningOptions, ImpactSummary, ApiKeys, UserProfile } from './types.ts';
import { useAuth } from './hooks/useAuth.tsx';
import Auth from './components/Auth.tsx';
import UserProfileDropdown from './components/UserProfile.tsx';
...
const MainAppContent: React.FC<MainAppContentProps> = ({ user }) => {
  ...
  useEffect(() => {
    if (!userId) return;
    const SESSION_LOG_KEY = `pageforge_ai_sessions_${userId}`;
    try {
  ...
  const handleMeasure = async (isRemeasure = false) => {
    if (!userId) { setApiError('User not authenticated. Please refresh the page.'); return; }
    ...
    try {
        const newReport = await fetchPageSpeedReport(url, userId);

        if (isRemeasure && pageSpeedBefore) {
            setPageSpeedAfter(newReport);
            const analysis = await generateComparisonAnalysis(pageSpeedBefore, newReport, userId);
            ...
            const plan = await generateOptimizationPlan(newReport, userId);
  ...
  const handleClean = useCallback(async () => {
    ...
    setApiError('');
    try {
        const { cleanedHtml: resultHtml, summary } = await cleanHtml(originalHtml, options, userId);
  ...
  }, [originalHtml, options, cleanHtml, isCleaning, userId]);
  ...
  return (
      <div className="space-y-8">
          ...
          <SetupGuide userId={userId} apiKeys={apiKeys} setApiKeys={setApiKeys} keysLoading={keysLoading}/>
          ...
          <SessionLog sessions={sessions} setSessions={setSessions} userId={userId} />
      </div>
  );
};
        ...
        <div className="max-w-7xl mx-auto w-full">
            <header className="mb-8 flex justify-between items-center">
              ...
              {user ? (
                ...
                <a href="https://github.com/nion-dev" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors">
