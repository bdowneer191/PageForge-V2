...
const ScoreDiff = ({ before, after }: { before: number, after: number }) => {
    ...
    return (
        <span>
            {Math.round(before)} &rarr; {Math.round(after)} <span className={`font-bold ${color}`}>({sign}{diff})</span>
        </span>
    );
}
...
const SessionLog: React.FC<SessionLogProps> = ({ sessions, setSessions, userId }) => {
    ...
    const SESSION_STORAGE_KEY = `pageforge_ai_sessions_${userId}`;
    ...
    const handleClear = async () => {
        if (!userId || sessions.length === 0 || !window.confirm('Are you sure you want to permanently delete all your session history? This action cannot be undone.')) {
            return;
        }
        setIsClearing(true);
        try {
                        ...
                        <div className="p-4 bg-gray-850 rounded-lg flex flex-wrap justify-between items-center gap-4">
                           ...
                           <div className="flex gap-2 flex-wrap">
                             ...
                             <button
                                   ...
                                   className="flex items-center justify-center gap-1.5 text-sm py-2 px-4 bg-blue-600 hover:bg-blue-700 rounded-md transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed text-white"
                               >
                                    ...
                                    <tbody>
                                        {displayedSessions.map(session => (
                                            <tr key={session.id || session.startTime} className="border-b border-gray-800 hover:bg-gray-850">
                                                ...
                                                <td className="py-2 pl-2"><ScoreDiff before={session.beforeScores.mobile*100} after={session.afterScores.mobile*100} /></td>
                                                <td className="py-2 pl-2"><ScoreDiff before={session.beforeScores.desktop*100} after={session.afterScores.desktop*100} /></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
